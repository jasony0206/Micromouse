...
void setup() {
  ...
  //INTERRUPT PINS
  attachInterrupt(INT_PIN1, gotTick, CHANGE);
}
...
boolean gotWallOnFront(){
  if(getIR(fleftEmt, fleftRcv) > WALL_VALUE)
    return true;
  return false;
}

boolean gotWallOnLeft(){
  if(getIR(leftEmt, leftRcv) > WALL_VALUE)
    return true;
  return false;
}

boolean gotWallOnRight(){
  if(getIR(rightEmt, rightRcv) > WALL_VALUE)
    return true;
  return false;
}
